...
for cfg in "${AP_JFR_CONFIGS[@]}"; do
  try verify_pair "$FIXTURES/java.async-profiler.$cfg.base.jfr" "$FIXTURES/java.async-profiler.$cfg.current.jfr"
done
try verify_pair "$FIXTURES/java.async-profiler.cpu.base.collapsed" "$FIXTURES/java.async-profiler.cpu.current.collapsed"
for cfg in "${JDK_JFR_CONFIGS[@]}"; do
  try verify_pair "$FIXTURES/java.jdk.$cfg.base.jfr" "$FIXTURES/java.jdk.$cfg.current.jfr"
done
