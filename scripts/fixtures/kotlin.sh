...
for cfg in "${AP_JFR_CONFIGS[@]}"; do
  try verify_pair "$FIXTURES/kotlin.async-profiler.$cfg.base.jfr" "$FIXTURES/kotlin.async-profiler.$cfg.current.jfr"
done
try verify_pair "$FIXTURES/kotlin.async-profiler.cpu.base.collapsed" "$FIXTURES/kotlin.async-profiler.cpu.current.collapsed"
for cfg in "${JDK_JFR_CONFIGS[@]}"; do
  try verify_pair "$FIXTURES/kotlin.jdk.$cfg.base.jfr" "$FIXTURES/kotlin.jdk.$cfg.current.jfr"
done
