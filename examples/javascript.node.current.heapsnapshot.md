...
| 0.0% | 2.33 kB |        97 | `TypeError`                  | `<unknown>`                          |
| 0.0% | 1.25 kB |        13 | `Float64Array`               | `<unknown>`                          |
| 0.0% | 1.22 kB |         8 | `Node / BindingData`         | `<unknown>`                          |
| 0.0% | 1.14 kB |        50 | `Node / std::basic_string`   | `<unknown>`                          |
| 0.0% |   904 B |        29 | `Map`                        | `<unknown>`                          |
| 0.0% |   896 B |        19 | `Promise`                    | `<unknown>`                          |
| 0.0% |   864 B |        18 | `Channel`                    | node:diagnostics_channel:182:14      |
| 0.0% |   864 B |        18 | `WeakReference`              | node:internal/util:889:14            |
| 0.0% |   776 B |        24 | `Set`                        | `<unknown>`                          |
| 0.0% |   752 B |         9 | `Uint8Array`                 | `<unknown>`                          |
| 0.0% |   752 B |         1 | `Node / PrincipalRealm`      | `<unknown>`                          |
| 0.0% |   744 B |        31 | `RangeError`                 | `<unknown>`                          |
...
|    % |    Size | Value                                                    | Path                                                                                            |
| ---: | ------: | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 0.0% | 1.95 kB | `import { readFileSync } from 'node:fs'\nimport { a…`    | `.source scripts/fixtures/assets/javascript/heap-snapshot.mjs ← .script (shared function info)` |
| 0.0% | 1.48 kB | `^(?:(?:(?:[0-9a-fA-F]{1,4}):){7}(?:(?:[0-9a-fA-F]…`     | `(GC root)`                                                                                     |
| 0.0% |   432 B | `TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA2…`     | `(GC root)`                                                                                     |
| 0.0% |   432 B | `TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA2…`     | `.--tls-cipher-list Object ← .optionsDict system / Context`                                     |
| 0.0% |   336 B | `アッサム山中の趣味用アカ。当分の間、選挙啓発用としても使っていきます。このアカウントがアッサム山中…`     | `.description Object ← .user Object ← .retweeted_status Object`                                 |
| 0.0% |   336 B | `ブリヂストンのスポーツタイヤ「POTENZA」のアカウントです。レースやタイヤの事などをつぶやきま…`     | `.description Object ← .user Object ← .retweeted_status Object`                                 |
| 0.0% |   336 B | `ROMって楽しんでいる部分もあり無言フォロー多めですすみません…。ツイート数多め・あらぶり多めなの…`     | `.description Object ← .user Object`                                                            |
| 0.0% |   336 B | `define unhandled rejections behavior. Options are…`     | `.helpText Object ← .345 array ← .table Map`                                                    |
| 0.0% |   336 B | `THE SECOND/劇団EXILE/EXILE/二代目JSB ☞KENCHI.AKIRA.青柳翔…`     | `.description Object ← .user Object`                                                            |
| 0.0% |   328 B | `ﾟ.＊97line おさらに貢いでる系女子＊.゜                         …`     | `.description Object ← .user Object`                                                            |
| 0.0% |   320 B | `ニコ動で踊り手やってます!!応援本当に嬉しいですありがとうございます!!　ぽっちゃりだけど前向きに…`     | `.description Object ← .user Object`                                                            |
| 0.0% |   312 B | `【無断転載禁止･コピペ禁止・非公式RT禁止】【必読！】⇒ http://t.co/nuUvfUVD…`     | `.description Object ← .user Object`                                                            |
| 0.0% |   304 B | `@aym0566x \n\n名前:前田あゆみ\n第一印象:なんか怖っ！\n今の印象:とりあえずキモい。噛み…` | `.text Object`                                                                                  |
| 0.0% |   304 B | `RT @AFmbsk: @samao21718 \n呼び方☞まおちゃん\n呼ばれ方☞あーちゃん\n第一印…`  | `.text Object`                                                                                  |
| 0.0% |   304 B | `ヤー・チャイカ。紫宝勢の末席くらいでQMAやってます。\n9/13（土）「九州杯」今年も宜しくお願…`    | `.description Object ← .user Object`                                                            |
| 0.0% |   304 B | `bot遊びと実況が主目的の趣味アカウント。成人済♀。時々TLお騒がせします。リフォ率低いですがＦ／…`     | `.description Object ← .user Object`                                                            |
| 0.0% |   296 B | `RT @omo_kko: ラウワン脱出→友達が家に連んで帰ってって言うから友達ん家に乗せて帰る(1…`     | `.text Object`                                                                                  |
| 0.0% |   296 B | `RT @shiawaseomamori: 一に止まると書いて、正しいという意味だなんて、この年にな…`     | `.text Object`                                                                                  |
| 0.0% |   296 B | `RT @shiawaseomamori: 一に止まると書いて、正しいという意味だなんて、この年にな…`     | `.text Object`                                                                                  |
| 0.0% |   296 B | `RT @shiawaseomamori: 一に止まると書いて、正しいという意味だなんて、この年にな…`     | `.text Object`                                                                                  |
