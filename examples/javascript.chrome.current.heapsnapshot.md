...
| 0.0% |   400 B |         1 | `Navigator`                   | `<unknown>` |
| 0.0% |   328 B |         1 | `FontFaceSet`                 | `<unknown>` |
| 0.0% |   208 B |         1 | `ScriptedAnimationController` | `<unknown>` |
| 0.0% |   200 B |         1 | `IntersectionObserver`        | `<unknown>` |
| 0.0% |   184 B |         1 | `DocumentTimeline`            | `<unknown>` |
...
|    % |  Size | Instances | Path                                                                                   |
| ---: | ----: | --------: | -------------------------------------------------------------------------------------- |
| 4.8% | 800 B |        10 | `[3] <p> ← [4] <article class="status"> ← .__retained Window / ://`                    |
| 4.3% | 720 B |         9 | `[3] <h2> ← [3] <article class="status"> ← .__retained Window / ://`                   |
| 1.0% | 160 B |         2 | `[3] <span class="hashtag"> ← [4] <article class="status"> ← .__retained Window / ://` |
| 0.5% |  80 B |         1 | `[3] <span class="hashtag"> ← .__retained Window / ://`                                |
...
|      % |  Size | Instances | Path                                                |
| -----: | ----: | --------: | --------------------------------------------------- |
| 100.0% | 960 B |         1 | `[1] InternalNode ← [1] InternalNode ← [23] Window` |
...
|      % |  Size | Instances | Path                                   |
| -----: | ----: | --------: | -------------------------------------- |
| 100.0% | 328 B |         1 | `[2] InternalNode ← [30] HTMLDocument` |
...
|      % |  Size | Instances | Path                                                      |
| -----: | ----: | --------: | --------------------------------------------------------- |
| 100.0% | 200 B |         1 | `[2] InternalNode ← [7] InternalNode ← [30] HTMLDocument` |
...
| 0.0% |   332 B | `ROMって楽しんでいる部分もあり無言フォロー多めですすみません…。ツイート数多め・あらぶり多めなの…`     | `(GC root)` |
| 0.0% |   332 B | `アッサム山中の趣味用アカ。当分の間、選挙啓発用としても使っていきます。このアカウントがアッサム山中…`     | `(GC root)` |
| 0.0% |   332 B | `ブリヂストンのスポーツタイヤ「POTENZA」のアカウントです。レースやタイヤの事などをつぶやきま…`     | `(GC root)` |
| 0.0% |   328 B | `THE SECOND/劇団EXILE/EXILE/二代目JSB ☞KENCHI.AKIRA.青柳翔…`     | `(GC root)` |
| 0.0% |   324 B | `ﾟ.＊97line おさらに貢いでる系女子＊.゜                         …`     | `(GC root)` |
| 0.0% |   312 B | `ニコ動で踊り手やってます!!応援本当に嬉しいですありがとうございます!!　ぽっちゃりだけど前向きに…`     | `(GC root)` |
| 0.0% |   304 B | `【無断転載禁止･コピペ禁止・非公式RT禁止】【必読！】⇒ http://t.co/nuUvfUVD…`     | `(GC root)` |
| 0.0% |   300 B | `bot遊びと実況が主目的の趣味アカウント。成人済♀。時々TLお騒がせします。リフォ率低いですがＦ／…`     | `(GC root)` |
| 0.0% |   296 B | `ヤー・チャイカ。紫宝勢の末席くらいでQMAやってます。\n9/13（土）「九州杯」今年も宜しくお願…`    | `(GC root)` |
| 0.0% |   292 B | `@samao21718 \n呼び方☞まおちゃん\n呼ばれ方☞あーちゃん\n第一印象☞平野から？！\n今の印…` | `(GC root)` |
| 0.0% |   292 B | `湯の街の元勃酩姦なんちゃら大　赤い犬の犬（外資系）　肥後で緑ナンバー屋さん勤め\nくだらないことしか…`    | `(GC root)` |
| 0.0% |   292 B | `一に止まると書いて、正しいという意味だなんて、この年になるまで知りませんでした。 人は生きていると…`     | `(GC root)` |
| 0.0% |   288 B | `家具（かぐ、Furniture）は、家財道具のうち家の中に据え置いて利用する比較的大型の道具類、ま…`     | `(GC root)` |
| 0.0% |   284 B | `●継続試合（中京対崇徳）46回～　9時～\n　〈ラジオ中継〉\n　らじる★らじる→大阪放送局を選択→N…`   | `(GC root)` |
| 0.0% |   280 B | `泉田新潟県知事は、東電の申請書提出を容認させられただけで、再稼働に必要な「同意」はまだ与えていませ…`     | `(GC root)` |
| 0.0% |   264 B | `ラウワン脱出→友達が家に連んで帰ってって言うから友達ん家に乗せて帰る(1度も行ったことない田舎道)…`     | `(GC root)` |
| 0.0% |   260 B | `自分なりに生きる人、最後まであきらめないの。でも、フォローありがとう…。@ringo_BDFFLO…`     | `(GC root)` |
| 0.0% |   260 B | `GパングのA型K月克己中尉の非公式botです。 主に七巻と八巻が中心の台詞をつぶやきます。 4/1…`     | `(GC root)` |
| 0.0% |   256 B | `福島第一原発の構内地図がこちら。\nhttp://t.co/ZkU4TZCGPG\nどう見ても、１号機…`   | `(GC root)` |
