...
| -------- | -----: | ----: | ------------: | --------: |
| stdlib   | -14.5% |   -52 | 97.5% → 98.4% | 358 → 306 |
...
|  +25.0% |    +1 | 1.1% → 1.6% |   4 → 5 | `hashCodeRange(int, int)`                                                                                     | `java.util.ArrayList`                                                |
|  +14.3% |    +1 | 1.9% → 2.6% |   7 → 8 | `getReturnState(int)`                                                                                         | `groovyjarjarantlr4.v4.runtime.atn.ArrayPredictionContext`           |
...
|  -25.0% |    -1 |   1.1% → 1.0% |   4 → 3 | `init(MemberName, Object)`                                                                   | `java.lang.invoke.MethodHandleNatives`                          |
...
|  +28.6% |    +4 |   3.8% → 5.8% | 14 → 18 | `get(Object)`                                                                           | `java.util.concurrent.ConcurrentHashMap`                                       |
| +133.3% |    +4 |   0.8% → 2.3% |   3 → 7 | `getCachedContext(PredictionContext, ConcurrentMap, PredictionContext$IdentityHashMap)` | `groovyjarjarantlr4.v4.runtime.atn.PredictionContext`                          |
|  +30.8% |    +4 |   3.5% → 5.5% | 13 → 17 | `join(PredictionContext, PredictionContext, PredictionContextCache)`                    | `groovyjarjarantlr4.v4.runtime.atn.PredictionContext`                          |
...
|  +18.2% |    +2 |   3.0% → 4.2% | 11 → 13 | `putVal(int, Object, Object, boolean, boolean)`                                         | `java.util.HashMap`                                                            |
...
|  -40.4% |   -21 | 14.2% → 10.0% |   52 → 31 | `newArray(Class, int)`                                                                      | `java.lang.reflect.Array`                            |
|  -35.0% |   -21 | 16.3% → 12.5% |   60 → 39 | `newInstance(Class, int)`                                                                   | `java.lang.reflect.Array`                            |
...
| -------- | -----: | -----------: | ------------: | ------------------: | ------------: |
| stdlib   |  -1.1% | -132.669 MiB | 99.1% → 99.2% | 11.8 GiB → 11.7 GiB | 6,223 → 6,219 |
| ours     |  -8.5% |   -9.026 MiB |   0.9% → 0.8% |  106 MiB → 97.2 MiB |       53 → 50 |
| native   |  -1.3% |       -464 B |         <0.1% |   34.4 KiB → 34 KiB |       47 → 46 |
...
|   +8.1% | +16.104 MiB |  1.6% → 1.8% |   200 MiB → 216 MiB | 100 → 110 | `allocateInstance(Object)`                                                                    | `java.lang.invoke.DirectMethodHandle`                                           |
|   +5.2% |   +12.9 MiB |  2.0% → 2.2% |   250 MiB → 263 MiB | 127 → 145 | `stream(Spliterator, boolean)`                                                                | `java.util.stream.StreamSupport`                                                |
|   +2.6% |  +11.25 MiB |  3.5% → 3.6% |   425 MiB → 436 MiB | 216 → 222 | `makeBlockInliningWrapper(MethodHandle)`                                                      | `java.lang.invoke.MethodHandleImpl`                                             |
|   +3.7% |  +9.087 MiB |  2.0% → 2.1% |   246 MiB → 255 MiB | 122 → 130 | `divideAndRemainderKnuth(BigInteger)`                                                         | `java.math.BigInteger`                                                          |
...
|   -2.5% |   -6.984 MiB |         2.3% |  280 MiB → 273 MiB | 142 → 143 | `make(MethodType, LambdaForm, Object, Object, Object, Object)`                          | `java.lang.invoke.BoundMethodHandle$Species_LLLL`                     |
...
|  +17.0% | +104.975 MiB |   5.1% → 6.0% |  616 MiB → 721 MiB |     307 → 359 | `newInstance(Class, int)`                                     | `java.lang.reflect.Array`                                                           |
...
|   -1.0% | -120.093 MiB | 94.4% → 94.5% | 11.2 GiB → 11.1 GiB | 5,889 → 5,884 | `fromCache(MutableCallSite, Class, String, int, Boolean, Boolean, Boolean, Object, Object[])` | `org.codehaus.groovy.vmplugin.v8.IndyInterface`                            |
...
|   -8.4% |  -73.529 MiB |   7.2% → 6.7% |   879 MiB → 805 MiB |     415 → 355 | `makeImpl(Class, Class[], boolean)`                                                           | `java.lang.invoke.MethodType`                                              |
...
| Category |  Change |        Delta |            % |               Size |   Samples |
| -------- | ------: | -----------: | -----------: | -----------------: | --------: |
| stdlib   |  -69.2% | -184.679 KiB |       100.0% | 267 KiB → 82.3 KiB | 116 → 111 |
