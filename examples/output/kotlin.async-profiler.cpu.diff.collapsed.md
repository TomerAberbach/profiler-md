...
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `subList`                     | `java.util.ArrayList`                                    |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `set`                         | `java.util.BitSet`                                       |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `hasNext`                     | `kotlin.sequences.TransformingSequence$iterator$1`       |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `next`                        | `kotlin.collections.ReversedListReadOnly$listIterator$1` |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `hasNext`                     | `kotlin.collections.builders.MapBuilder$Itr`             |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `hasNext`                     | `java.util.Collections$1`                                |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `isEmpty`                     | `java.util.AbstractCollection`                           |
|     new |    +1 | 0.0% → 0.1% |   0 → 1 | `checkNotNullExpressionValue` | `kotlin.jvm.internal.Intrinsics`                         |
...
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `charAt`           | `java.lang.String`                                    |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `reverse`          | `kotlin.collections.CollectionsKt___CollectionsJvmKt` |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `add`              | `java.util.ArrayList`                                 |
...
|  +75.0% |    +3 |   0.3% → 0.5% |     4 → 7 | `linkMethodHandleConstant` | `java.lang.invoke.MethodHandleNatives`             |
|     new |    +3 |   0.0% → 0.2% |     0 → 3 | `substring`                | `java.lang.String`                                 |
|  +66.7% |    +2 |   0.2% → 0.4% |     3 → 5 | `linkToTargetMethod`       | `java.lang.invoke.Invokers$Holder`                 |
| +100.0% |    +2 |   0.1% → 0.3% |     2 → 4 | `hasNext`                  | `kotlin.sequences.FilteringSequence$iterator$1`    |
| +200.0% |    +2 |   0.1% → 0.2% |     1 → 3 | `hashCode`                 | `java.lang.Object`                                 |
|     new |    +2 |   0.0% → 0.1% |     0 → 2 | `iterator`                 | `java.util.ArrayList`                              |
|     new |    +2 |   0.0% → 0.1% |     0 → 2 | `iterator`                 | `kotlin.collections.builders.ListBuilder`          |
|     new |    +2 |   0.0% → 0.1% |     0 → 2 | `newDirectoryStream`       | `java.nio.file.Files`                              |
|     new |    +2 |   0.0% → 0.1% |     0 → 2 | `zip`                      | `kotlin.collections.CollectionsKt___CollectionsKt` |
|  +33.3% |    +1 |   0.2% → 0.3% |     3 → 4 | `toByteArray`              | `java.io.ByteArrayOutputStream`                    |
|   +7.1% |    +1 |   1.0% → 1.1% |   14 → 15 | `getValue`                 | `kotlin.UnsafeLazyImpl`                            |
| +100.0% |    +1 |          0.1% |     1 → 2 | `toList`                   | `kotlin.collections.CollectionsKt___CollectionsKt` |
| +100.0% |    +1 |          0.1% |     1 → 2 | `getAnnotation`            | `java.lang.reflect.Field`                          |
|     new |    +1 |   0.0% → 0.1% |     0 → 1 | `toMap`                    | `kotlin.collections.MapsKt__MapsKt`                |
|     new |    +1 |   0.0% → 0.1% |     0 → 1 | `removeAll`                | `java.util.ArrayList`                              |
|     new |    +1 |   0.0% → 0.1% |     0 → 1 | `hasNext`                  | `java.util.LinkedHashMap$LinkedHashIterator`       |
...
| removed |    -3 | 0.2% → 0.0% |   3 → 0 | `findMethodHandleType`     | `java.lang.invoke.MethodHandleNatives`                |
| removed |    -2 | 0.1% → 0.0% |   2 → 0 | `getKey`                   | `java.util.HashMap$Node`                              |
| removed |    -2 | 0.1% → 0.0% |   2 → 0 | `getClass`                 | `java.lang.Object`                                    |
| removed |    -2 | 0.1% → 0.0% |   2 → 0 | `hashCode`                 | `java.util.Arrays`                                    |
| removed |    -2 | 0.1% → 0.0% |   2 → 0 | `put`                      | `java.util.concurrent.ConcurrentHashMap`              |
| removed |    -2 | 0.1% → 0.0% |   2 → 0 | `hasNext`                  | `kotlin.collections.EmptyIterator`                    |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `filter`                   | `kotlin.sequences.SequencesKt___SequencesKt`          |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `toSet`                    | `kotlin.collections.CollectionsKt___CollectionsKt`    |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `end`                      | `java.util.zip.Inflater`                              |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `getCurrentThreadUserTime` | `sun.management.ThreadImpl`                           |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `closeFinally`             | `kotlin.io.CloseableKt`                               |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `subSequence`              | `java.lang.StringBuilder`                             |
| removed |    -1 | 0.1% → 0.0% |   1 → 0 | `next`                     | `kotlin.sequences.GeneratorSequence$iterator$1`       |
|  -25.0% |    -1 | 0.3% → 0.2% |   4 → 3 | `get`                      | `java.util.HashMap`                                   |
