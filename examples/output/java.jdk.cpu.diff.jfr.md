...
| +200.0% |    +2 |   0.1% → 0.2% |     1 → 3 | `<clinit>()`                        | `scala.Predef$`                      |
|     new |    +2 |   0.0% → 0.1% |     0 → 2 | `toArray(IntFunction)`              | `java.util.stream.ReferencePipeline` |
|  +14.3% |    +1 |   0.4% → 0.5% |     7 → 8 | `foreach(Function1)`                | `scala.collection.immutable.List`    |
|  +25.0% |    +1 |   0.2% → 0.3% |     4 → 5 | `collect(Collector)`                | `java.util.stream.ReferencePipeline` |
|  +20.0% |    +1 |   0.3% → 0.4% |     5 → 6 | `<init>(Map)`                       | `java.util.HashMap`                  |
