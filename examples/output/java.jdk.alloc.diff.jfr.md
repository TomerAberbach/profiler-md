...
|  -16.7% |    -2 |   0.7% → 0.6% |       12 → 10 | `forEach(BiConsumer)`               | `java.util.HashMap`                 |
| removed |    -1 |   0.1% → 0.0% |         1 → 0 | `<clinit>()`                        | `scala.runtime.LazyVals$`           |
|  -12.5% |    -1 |   0.5% → 0.4% |         8 → 7 | `foreach(Function1)`                | `scala.collection.immutable.List`   |
...
| +200.0% |       +80 B |        <0.1% |   40 B → 120 B |   1 → 3 | `apply(int)`                                   | `org.renaissance.jdk.concurrent.JavaKMeans$$Lambda.0x000000b8011a1d48` |
| +200.0% |       +80 B |        <0.1% |   40 B → 120 B |   1 → 3 | `lambda$generateData$4(int)`                   | `org.renaissance.jdk.concurrent.JavaKMeans`                            |
|  +75.0% |       +48 B |        <0.1% |   64 B → 112 B |       1 | `createParser(Map)`                            | `org.renaissance.harness.ConfigParser`                                 |
|  +75.0% |       +48 B |        <0.1% |   64 B → 112 B |       1 | `<init>(Map)`                                  | `org.renaissance.harness.ConfigParser`                                 |
|     new |       +48 B | 0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `loadProperties(URL)`                          | `org.renaissance.core.ResourceUtils`                                   |
|     new |       +48 B | 0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `loadPropertiesAsMap(URL)`                     | `org.renaissance.core.ResourceUtils`                                   |
|     new |       +48 B | 0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `create(Path, String, Optional, Map, boolean)` | `org.renaissance.core.BenchmarkSuite`                                  |
|     new |       +48 B | 0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `$anonfun$1(Config, Path)`                     | `org.renaissance.harness.RenaissanceSuite$`                            |
|     new |       +48 B | 0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `apply()`                                      | `org.renaissance.harness.RenaissanceSuite$$$Lambda.0x000000b80117e000` |
...
|    new | +98.062 KiB |   0.0% → 2.4% | 0 B → 98.1 KiB |   0 → 1 | `<init>(Collection)`                | `java.util.ArrayList`                |
|    new |      +112 B |  0.0% → <0.1% |    0 B → 112 B |   0 → 1 | `<clinit>()`                        | `scala.runtime.LazyVals$`            |
|    new |       +48 B |  0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `load(InputStream)`                 | `java.util.Properties`               |
|    new |       +48 B |  0.0% → <0.1% |     0 B → 48 B |   0 → 1 | `apply(Function0)`                  | `scala.util.Try$`                    |
|  +5.9% |        +8 B |         <0.1% |  136 B → 144 B |   5 → 4 | `toArray(IntFunction)`              | `java.util.stream.ReferencePipeline` |
|    ~0% |        +8 B | 80.5% → 51.6% |       2.06 MiB |   6 → 5 | `collect(Collector)`                | `java.util.stream.ReferencePipeline` |
|    ~0% |        +8 B | 80.5% → 51.6% |       2.06 MiB |   6 → 5 | `foreach(Function1)`                | `scala.collection.immutable.List`    |
