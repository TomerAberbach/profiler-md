...
Took 1.29s → 1.24s (-0.05s, -3.4%) over 185 samples → 173 samples (7.0ms → 7.2ms per sample).
...
|   -3.1% | -34.0ms | 84.2% → 84.4% |     1.08s → 1.05s | 132 → 133 | `ZSTD_compressContinue_public`   | `<unknown>`       |
|   -2.0% | -19.0ms | 73.7% → 74.8% | 951.0ms → 932.0ms |   83 → 82 | `ZSTD_btGetAllMatches_noDict_3`  | zstd_opt.c        |
|   -5.1% | -10.0ms | 15.3% → 15.1% | 198.0ms → 188.0ms |   48 → 34 | `ZSTD_compressEnd_public`        | `<unknown>`       |
|  -11.1% |  -2.0ms |   1.4% → 1.3% |   18.0ms → 16.0ms |   15 → 13 | `ZSTD_compressBlock_btultra2`    | `<unknown>`       |
|  -40.0% |  -2.0ms |   0.4% → 0.2% |     5.0ms → 3.0ms |     4 → 3 | `ZSTD_updateStats`               | zstd_opt.c        |
| removed |  -1.0ms |   0.1% → 0.0% |       1.0ms → 0ms |     1 → 0 | `ZSTDMT_initCStream_internal`    | `<unknown>`       |
