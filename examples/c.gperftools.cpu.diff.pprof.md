...
Took 1.29s → 1.24s (-44.0ms, -3.4%) over 1,290 samples → 1,246 samples (1.0ms per sample).

| Category | Change |   Delta |             % |            Time |       Samples |
| -------- | -----: | ------: | ------------: | --------------: | ------------: |
| ours     |  -3.7% | -47.0ms | 99.1% → 98.9% |   1.27s → 1.23s | 1,279 → 1,232 |
| stdlib   | +27.3% |  +3.0ms |   0.9% → 1.1% | 11.0ms → 14.0ms |       11 → 14 |
...
|  +42.9% | +3.0ms | 0.5% → 0.8% | 7.0ms → 10.0ms |  7 → 10 | `ZSTD_litLengthPrice.constprop.1.isra.0` | zstd_opt.c        |
| +100.0% | +3.0ms | 0.2% → 0.5% |  3.0ms → 6.0ms |   3 → 6 | `ZSTD_litLengthPrice.constprop.0.isra.0` | zstd_opt.c        |
...
|  Change |   Delta |             % |              Time |   Samples | Function                                             | Location          |
| ------: | ------: | ------------: | ----------------: | --------: | ---------------------------------------------------- | ----------------- |
|   -9.8% | -30.0ms | 23.7% → 22.2% | 306.0ms → 276.0ms | 306 → 276 | `ZSTD_compressBlock_opt2`                            | zstd_opt.c        |
|   -2.3% | -22.0ms | 73.6% → 74.4% | 949.0ms → 927.0ms | 949 → 927 | `ZSTD_btGetAllMatches_noDict_3`                      | zstd_opt.c        |
|  -40.0% |  -2.0ms |   0.4% → 0.2% |     5.0ms → 3.0ms |     5 → 3 | `ZSTD_updateStats`                                   | zstd_opt.c        |
...
|  Change |  Delta |           % |           Time | Samples | Function                                             | Location                  |
| ------: | -----: | ----------: | -------------: | ------: | ---------------------------------------------------- | ------------------------- |
| +200.0% | +6.0ms | 0.2% → 0.7% |  3.0ms → 9.0ms |   3 → 9 | `ZSTD_buildBlockEntropyStats`                        | `<unknown>`               |
| +133.3% | +4.0ms | 0.2% → 0.6% |  3.0ms → 7.0ms |   3 → 7 | `ZSTD_seqToCodes`                                    | `<unknown>`               |
|  +42.9% | +3.0ms | 0.5% → 0.8% | 7.0ms → 10.0ms |  7 → 10 | `ZSTD_litLengthPrice.constprop.1.isra.0`             | zstd_opt.c                |
...
| +100.0% | +1.0ms | 0.1% → 0.2% |  1.0ms → 2.0ms |   1 → 2 | `ZSTD_encodeSequences`                               | `<unknown>`               |
| +100.0% | +1.0ms | 0.1% → 0.2% |  1.0ms → 2.0ms |   1 → 2 | `ZSTD_compressBegin_internal`                        | zstd_compress.c           |
| +100.0% | +1.0ms | 0.1% → 0.2% |  1.0ms → 2.0ms |   1 → 2 | `ZSTD_compressBegin_advanced_internal`               | `<unknown>`               |
...
|  Change |   Delta |             % |              Time |       Samples | Function                         | Location          |
| ------: | ------: | ------------: | ----------------: | ------------: | -------------------------------- | ----------------- |
|   -3.9% | -49.0ms | 97.4% → 97.0% |     1.25s → 1.20s | 1,257 → 1,208 | `ZSTD_compressBlock_opt2`        | zstd_opt.c        |
|   -3.6% | -46.0ms | 98.7% → 98.5% |     1.27s → 1.22s | 1,273 → 1,227 | `ZSTD_buildSeqStore`             | zstd_compress.c   |
|   -3.4% | -44.0ms |         99.5% |     1.28s → 1.24s | 1,284 → 1,240 | `ZSTD_compress_frameChunk`       | zstd_compress.c   |
|   -3.3% | -43.0ms | 99.7% → 99.8% |     1.28s → 1.24s | 1,286 → 1,243 | `ZSTDMT_compressionJob`          | zstdmt_compress.c |
|   -3.3% | -43.0ms | 99.7% → 99.8% |     1.28s → 1.24s | 1,286 → 1,243 | `POOL_thread`                    | pool.c            |
|   -3.1% | -34.0ms | 84.2% → 84.4% |     1.08s → 1.05s | 1,086 → 1,052 | `ZSTD_compressContinue_public`   | `<unknown>`       |
|   -2.0% | -19.0ms | 73.7% → 74.8% | 951.0ms → 932.0ms |     951 → 932 | `ZSTD_btGetAllMatches_noDict_3`  | zstd_opt.c        |
|   -5.1% | -10.0ms | 15.3% → 15.1% | 198.0ms → 188.0ms |     198 → 188 | `ZSTD_compressEnd_public`        | `<unknown>`       |
|  -11.1% |  -2.0ms |   1.4% → 1.3% |   18.0ms → 16.0ms |       18 → 16 | `ZSTD_compressBlock_btultra2`    | `<unknown>`       |
|  -40.0% |  -2.0ms |   0.4% → 0.2% |     5.0ms → 3.0ms |         5 → 3 | `ZSTD_updateStats`               | zstd_opt.c        |
| removed |  -1.0ms |   0.1% → 0.0% |       1.0ms → 0ms |         1 → 0 | `ZSTDMT_initCStream_internal`    | `<unknown>`       |
...
| removed |  -1.0ms |   0.1% → 0.0% |       1.0ms → 0ms |         1 → 0 | `ZSTD_setBasePrices`             | zstd_opt.c        |
| removed |  -1.0ms |   0.1% → 0.0% |       1.0ms → 0ms |         1 → 0 | `ZSTD_buildCTable`               | `<unknown>`       |
