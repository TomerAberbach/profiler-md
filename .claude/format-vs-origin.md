...
2. From which origins and formats that set spans, decide what the behavior is a
   quirk of, and place the logic there:
...
- **Independent convergence suggests an idiom.** When unrelated origins or
  ...
  likely converge on that convention. Expect more occurrences from origins
  without committed inputs, and prefer the broader placement so each new one is
  already handled
...
- **Harm from over-applying.** Broad placement is acceptable only when applying
  in an unanticipated scenario is harmless (e.g. the rule removes only pure
