...
| `--function`        | off          | Record the first line of each function in place of the executing line. py-spy's output omits the mode, so each function's `Lines` section lists the definition line as if it were sampled |
