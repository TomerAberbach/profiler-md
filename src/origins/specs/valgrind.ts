...
export const valgrindOriginSpec = {
  ...
  // (`X → _dl_runtime_resolve → Y` on each first call), so its outgoing arcs
  // fuse unrelated functions into a single spurious recursion cycle. Dropping
  // them makes it a leaf, the same result the callgrind manual's
  // `--fn-skip=_dl_runtime_resolve*` advice achieves at capture time.
  dropsCallGraphArcs: ({ name }) =>
