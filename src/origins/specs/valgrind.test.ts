...
describe(`dropsCallGraphArcs`, () => {
  const { dropsCallGraphArcs } = valgrindOriginSpec

  test(`drops the dynamic linker's lazy-binding trampoline's arcs`, () => {
    expect(
      dropsCallGraphArcs(
        absoluteEntry(
  ...
  test(`keeps an ordinary function's arcs`, () => {
    expect(dropsCallGraphArcs(relativeEntry(`main`, `/src/main.c`))).toBe(false)
  })

  test(`keeps a nameless frame's arcs`, () => {
    expect(dropsCallGraphArcs({ id: 1 })).toBe(false)
  })
})
