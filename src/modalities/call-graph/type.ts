 ...
 * plus caller→callee arcs with recorded call counts and inclusive costs, and
 * no call stacks (the format records none). From here, origin detection,
 * frame normalization, and aggregation all run uniformly, so a format's only
 * custom logic is parsing into this.
 */
...
export type CallGraphFunction = {
  ...
  /** 1-based line to the self cost recorded at that line, per metric. */
